...
// ---------------------------------------------------------------------------
// Tests: Availability endpoint (booking-links/[id]/availability/route.ts)
// ---------------------------------------------------------------------------

describe('GET /api/booking-links/:id/availability', () => {
  beforeEach(() => vi.clearAllMocks())
  ...
  it('returns 400 when start/end params are missing', async () => {
    ...
    const url = 'http://localhost/api/booking-links/bl-1/availability'
    const res = await GET(
      new NextRequest(url),
      { params: Promise.resolve({ id: 'bl-1' }) },
    )
  ...
  it('returns 400 for invalid date format', async () => {
    const { GET } = await import('../[id]/availability/route')
    const url = 'http://localhost/api/booking-links/bl-1/availability?start=June1&end=June7'
    const res = await GET(
      new NextRequest(url),
      { params: Promise.resolve({ id: 'bl-1' }) },
    )
  ...
  it('returns 404 when booking link does not exist', async () => {
    serviceClientFactory = () => ({
      from: vi.fn().mockReturnValue(createChain({ data: null, error: { code: 'PGRST116' } })),
    })

    const mod = await import('../[id]/availability/route')
    const url = 'http://localhost/api/booking-links/bl-1/availability?start=2026-06-16&end=2026-06-20'
  ...
  it('returns 404 when booking link is not public', async () => {
    const link = { ...BOOKING_LINK_ROW, is_public: false }
    serviceClientFactory = () => ({
      from: vi.fn().mockReturnValue(createChain({ data: link, error: null })),
    })
...
// ---------------------------------------------------------------------------
// Tests: Book endpoint (booking-links/[id]/book/route.ts)
// ---------------------------------------------------------------------------

describe('POST /api/booking-links/:id/book', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    serviceClientFactory = () => ({})
  })
  ...
  it('returns 400 for invalid JSON', async () => {
    ...
    const { POST } = await import('../[id]/book/route')
    const req = new NextRequest('http://localhost/api/booking-links/bl-1/book', {
      method: 'POST',
      body: 'not json',
      headers: { 'Content-Type': 'application/json' },
    })
    const res = await POST(req, { params: Promise.resolve({ id: 'bl-1' }) })

    expect(res.status).toBe(400)
  })

  it('returns 400 for missing booker fields', async () => {
    serviceClientFactory = () => ({
      from: vi.fn().mockReturnValue(createChain({ data: null, error: null })),
    })

    const { POST } = await import('../[id]/book/route')
    const req = makeRequest('POST', { booker_name: 'Alice' }, 'http://localhost/api/booking-links/bl-1/book')
    const res = await POST(req, { params: Promise.resolve({ id: 'bl-1' }) })
  ...
  it('returns 404 when booking link does not exist', async () => {
    ...
    }, 'http://localhost/api/booking-links/bl-1/book')
    const res = await POST(req, { params: Promise.resolve({ id: 'bl-1' }) })

    expect(res.status).toBe(404)
  })

  it('returns 409 when slot is outside minimum notice window', async () => {
    const link = {
      ...BOOKING_LINK_ROW,
    ...
    const { POST } = await import('../[id]/book/route')
    // Book for 1 hour from now -- fails 48-hour minimum notice
  ...
  it('returns 409 when slot is outside availability window', async () => {
    // Availability is Monday 09:00-17:00, but we book for a time outside that
    const link = {
      ...BOOKING_LINK_ROW,
      minimum_notice_hours: 0,
      is_public: true,
      availability: [{ day: 'monday', start: '09:00', end: '10:00' }],
    }
    serviceClientFactory = () => ({
      from: vi.fn().mockReturnValue(createChain({ data: link, error: null })),
    })
    ...
    const { POST } = await import('../[id]/book/route')
    // Pick a future Tuesday at 14:00 UTC -- not Monday
    const now = new Date()
    const daysUntilTuesday = (2 - now.getDay() + 7) % 7 || 7
    const tuesday = new Date(now)
    tuesday.setDate(tuesday.getDate() + daysUntilTuesday)
    tuesday.setHours(14, 0, 0, 0)
    const tuesdayIso = tuesday.toISOString()

    const req = makeRequest('POST', {
      booker_name: 'Bob',
      booker_email: 'bob@example.com',
      start_time: tuesdayIso,
    }, 'http://localhost/api/booking-links/bl-1/book')
    const res = await POST(req, { params: Promise.resolve({ id: 'bl-1' }) })
    const json = await res.json()

    expect(res.status).toBe(409)
    expect(json.error).toContain('outside available hours')
  })
})
