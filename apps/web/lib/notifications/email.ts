...
function buildHtml(payload: NotificationPayload): string {
  ...
  const cta = payload.url
    ? `<p style="margin-top:16px"><a href="${sanitizeUrl(payload.url)}" style="display:inline-block;padding:10px 20px;background:#6366f1;color:#fff;border-radius:6px;text-decoration:none;font-size:14px">Open in Meowlendar</a></p>`
    : ''
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1a1a1a">
  ...
  <p style="font-size:11px;color:#999;margin:0">Meowlendar Notifications</p>
</body>
</html>`.trim()
}
...
export async function sendEmailNotification(
  ...
  try {
    const { error } = await getResend().emails.send({
      from: process.env.RESEND_FROM_EMAIL || 'Meowlendar <noreply@meowlendar.com>',
      to,
